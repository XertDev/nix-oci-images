...
let
  ...
  image = { package ?
    ...
    , bind ? "0.0.0.0", port ? 5000, extraPackages ? (_: [ ])
    , customLovelaceModules ? [ ], defaultComponents ? [ ]
    , customComponents ? [ ] }:
    dockerTools.streamLayeredImage (let
      ...
      hashParts = [ (toString bind) (toString port) ] ++ components
        ++ ((finalPackage.extraPackages or (_: [ ])) finalPackage.python.pkgs)
        ++ [ configFile.outPath ] ++ customLovelaceModules
    ...
    in {
      ...
      passthru = {
        ...
        configOverrideFile = MANUAL_CONFIG;
